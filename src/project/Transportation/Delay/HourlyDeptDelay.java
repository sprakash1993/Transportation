...
public class HourlyDeptDelay {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String[] nextLine = value.toString().split(",");
				...
				if (!nextLine[DELAY].isEmpty() && nextLine[DELAY] != null) {
					String hr=nextLine[HOUR].replace("\"", "").substring(0,2);
					if(hr.equals("24"))
						hr="00";
					context.write(new Text(hr), new Text(nextLine[DELAY]));
				}
	...
	public static void main(String[] args) throws Exception{
		...
		job.setJarByClass(HourlyDeptDelay.class);
