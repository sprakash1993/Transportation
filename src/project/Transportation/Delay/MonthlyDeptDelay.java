...
import project.Transportation.Delay.MonthlyArrDelay.AirlineMapper;
import project.Transportation.Delay.MonthlyArrDelay.AirlineReducer;

public class MonthlyDeptDelay {

	public static class AirlineMapper extends Mapper<LongWritable, Text, Text, Text> {
		static int DELAY = 28, MONTH=2;
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			String[] nextLine = value.toString().split(",");
				...
				if (!nextLine[DELAY].isEmpty() && nextLine[DELAY] != null) {
					context.write(new Text(nextLine[MONTH]), new Text(nextLine[DELAY]));
				}
		}
	}
	...
	public static void main(String[] args) throws Exception{
		...
		job.setJarByClass(MonthlyDeptDelay.class);
